"""
BERTweet Sentiment Analysis on SST-2 (GLUE)
- AdamW optimizer
...
NUM_LABELS   = 2
MAX_LEN      = 64     # SST-2 sentences are short; 64 tokens is sufficient
BATCH_SIZE   = 16
NUM_EPOCHS   = 2
N_FOLDS      = 2
...
def load_sst2_balanced(split, n):
    """
    Load n balanced samples from SST-2 split.
    SST-2 train is shuffled so we filter by label to guarantee balance.
    The validation split (872 samples) is used as-is for final evaluation.
    """
    raw  = load_dataset("glue", "sst2")
    data = raw[split]
    if n is None:
        # Return all (used for validation/test split)
        return data["sentence"], data["label"]
    half    = n // 2
    neg_idx = [i for i, l in enumerate(data["label"]) if l == 0][:half]
    pos_idx = [i for i, l in enumerate(data["label"]) if l == 1][:half]
    indices = neg_idx + pos_idx
    texts  = [data["sentence"][i] for i in indices]
...
def main():
    ...
    # Load data
    # SST-2 test labels are hidden in GLUE; use the validation split for evaluation
    print("Loading SST-2 (GLUE) ...")
    train_texts, train_labels = load_sst2_balanced("train",      SUBSET_TRAIN)
    test_texts,  test_labels  = load_sst2_balanced("validation", None)

    print(f"Train: {len(train_texts)} samples  |  "
          f"Neg={train_labels.count(0) if isinstance(train_labels, list) else (np.array(train_labels)==0).sum()}  "
          f"Pos={train_labels.count(1) if isinstance(train_labels, list) else (np.array(train_labels)==1).sum()}")
    print(f"Test:  {len(test_texts)} samples  |  "
          f"Neg={(np.array(test_labels)==0).sum()}  "
          f"Pos={(np.array(test_labels)==1).sum()}")
    ...
    for lr in LEARNING_RATES:
        print(f"\n{'='*55}")
        print(f"LR = {lr}")
        full_ds   = SST2Dataset(train_texts, train_labels, tokenizer, MAX_LEN)
        fold_accs = []
    ...
    train_ds   = SST2Dataset(train_texts, train_labels, tokenizer, MAX_LEN)
    test_ds    = SST2Dataset(test_texts,  test_labels,  tokenizer, MAX_LEN)
    ...
    report_dict = classification_report(
        truths, preds, labels=[0, 1], target_names=LABEL_NAMES,
        output_dict=True, zero_division=0
    )
    report_str = classification_report(
        truths, preds, labels=[0, 1], target_names=LABEL_NAMES, zero_division=0
    )
    print(report_str)
    ...
    ax.set_title("Confusion Matrix -- BERTweet on SST-2")
    plt.tight_layout()
